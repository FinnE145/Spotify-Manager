...
# -- S_sweep.md §3.4 D: `or`-default lines that only bite on a NULL/falsy
# stored value ----------------------------------------------------------
#
# builders.UNSET exists exactly for this shape: `None` must mean a real SQL
# NULL, never "use the builder's default", or a test naming the NULL path
# would silently exercise a different one and still pass.

import entities


def test_tenure_page_clamps_page_zero_and_a_missing_page_param_to_one(client, conn, monkeypatch):
    # source: S_sweep.md §3.4 D -- app.py:706's `or 1` only has an effect when
    # `?page=` resolves to the falsy int 0 (col 57's mutant), and its `1,
    # type=int` default only has an effect when `page` is absent from the
    # query string at all (col 41's mutant, unmasked by `or 1` because 2 is
    # truthy). _TENURE_PAGE_SIZE is monkeypatched down to 1 row per page so
    # two groups are enough to need two pages, rather than the 101 real ones
    ...
    # sanity: g1 must sort first (lower group_id) for the assertions below to
    # mean what they say -- both groups tie on tenure length (one generation
    # each), so the group_id tiebreak is what decides page order.
    assert g1["version"] < g2["version"]

    resp_zero = client.get("/dev/generations/tenure?page=0")
    assert "Page 1 of 2" in resp_zero.get_data(as_text=True)

    resp_absent = client.get("/dev/generations/tenure")
    assert "Page 1 of 2" in resp_absent.get_data(as_text=True)
...
def test_play_stats_null_month_and_week_display_as_zero_not_none(conn):
    ...
    # belongs. A target track with zero plays, alongside a recent play on a
    # *different* track (so data_through stays recent and the separate
    # staleness override at lines 67-70 does not also turn month/week to
    # None for its own reason), isolates exactly the `or 0` line.
