...
| **P3** — refactor | `docs/codebase-health/P3_refactor.md` | **Done and verified 2026-08-22. All three sessions landed and merged on their own, then P3 as a whole got the Verify pass §8 promises it; 861 tests, `conn.execute` in `app.py` 42 → 13, no import cycle, and the golden snapshots deleted.** **P3's Verify pass** re-ran the compare (zero diffs) and 22 of P3-007's mutations (all 22 caught) rather than reading either result, then deleted the baseline — the one irreversible step, held back from session 3 for exactly that reason. It found **P3-008** by changing the unit of mutation: P3-007 swept every payload *key*, which answers "is this read?" and never "is this right?", and three rules inside keys it had already ticked off survived the suite — `tenure_page`'s tier echo (all five of session 3's new tenure tests pass `tier="version"`, so none could tell an echoed argument from a constant, while `generations_tenure.html:52` pairs that tier with each row's group id) and both halves of §11.1's rank-before-cap, on `/dev/canonical?search=` and on `/search`'s songs. The middle one was caught by **nothing** — `routes_catalog.py:267` is `?search=zzz`, which matches too little to reorder — and is P3-007's own catalog-reason result recurring. Three assertions fixed. The carry-forward is that P3's four sweeps never stopped finding one class; they only changed the granularity they looked at it from. **Session 3** extracted the three dev pages — `/dev/canonical` → `canonical_detect.index_data`, `/dev/snapshot` → `snapshot.index_data`, `/dev/generations/tenure` → `entities.tenure_page` — finishing at `conn.execute` in `app.py` **13**, `app.py` 1,077 lines, and zero golden diffs. Then the close-out: `CLAUDE.md`'s map, §4.3's module-list check, and the spec updates §7 asks for — four passages across `entity-pages-K.md`, `async-recompute-N.md` and `scoring-H.md`, which is all the grep turned up that extraction actually falsifies. **The snapshots are the one §8 item session 3 did not do**: Finn ruled them over to P3's Verify pass, since both earlier Verify passes produced their findings by re-running the compare and the measurement rather than reading the session's account, and §3.4 forbids re-capturing anything session 3 deleted. **Two findings.** **P3-006** is a defect in §4.1's own table: it sends `tenure_page` to `generations.py`, which cannot have it, because the page ranks by score before paginating and `scoring.py` already imports `generations` — so the named home closes a cycle in the step whose §8 criterion 3 forbids one. Ruled into `entities.py`, which imports both and so costs no new edge. Its generalization is about the table rather than the row: **ownership and the import graph are two constraints, and §4.1.1 checked both for the one view that visibly had no clean home while the other eight got the ownership rule alone.** **P3-007** is P3-005's assignment discharged — 38 mutations over the three payloads and their route wiring, suite and compare run separately so each green was attributable: **16 held by golden alone, 4 by nothing**, and 42% of the dev pages' payload keys were golden-only against 19% of the entity pages', which is P2 sessions 4 and 5 having aimed at the entity pages and the route layer and never at these. Two of the four unobserved results were **not missing tests**: `liked_playlist_id` is dead (one hit in the tree, the line that makes it; pre-existing on `main`; ruled record-not-remove exactly like `track_scores`), and "a search lifts the listing cap" is unobservable for a *catalog* reason, since every `?q=` case matches fewer rows than the cap of 50. 26 tests later, **19 of the 20 uncaught mutations die against the permanent suite and the twentieth is the dead key** — so the golden suite was redundant before it was deleted, which is the finish line P3-004 named rather than merely a clean compare. **Session 2** extracted the six entity pages plus `generation_view`, `conn.execute` 42 → 19, and found P3-004/P3-005. Session 1 built the §3 golden harness, proved §3.3's gate, captured the baseline once, and then made the three small independent changes — §4.4's deletion, §4.2's cycle, §4.5's `SELECT *` — coming out at **zero golden diffs**. Its Verify pass re-ran the compare independently and then mutation-tested the harness itself, on the reasoning that a green compare is the *entire* evidence for the ~583 lines sessions 2 and 3 move: dropping a column from `_board_state`'s new list is caught and named. **Three findings, all ruled** (`P3_findings.md`). **P3-001 is the one to read** — it corrects its own first version, which had concluded from the physical column-order difference between a migrated and a fresh database that naming the columns in migrated order was *forced*; Flask's `app.json.sort_keys` is `True`, so `jsonify` had been discarding insertion order all along and the constraint never existed. The clean compare was read as confirming a mechanism that cannot reach the bytes it compares. **P3-002** records what the baseline is therefore blind to (JSON key order; status codes), recorded rather than fixed because the only route into an existing baseline is the re-capture §3.4 forbids. **P3-003** is Verify's, and is the sharpest: deleting `strip_accents` from the newly-extracted `normalize.base_string` passed all 770 tests **and** compared clean — a real hole, since `symr.db` carries a merged "Jerome Ducros"/"Jérôme Ducros" pair that only ever bucketed together through that call. Not a P3 regression (`main` has it identically), and the assertion is fixed, but it names the intersection worth watching for the rest of P3: **a golden baseline over a curated library observes the outcome of past curation, not the rule that produced it** — anything already merged, reviewed, pinned or aliased renders from a stored row, so the rule is not re-run on the page being compared. Three sessions on `feat/codebase-health-P`: (1) build the golden harness, prove the determinism gate, capture, then the small independent fixes; (2) the six entity pages; (3) the three dev pages plus close-out. **Nine views extracted, ~583 sloc**, taking `create_app` 1,620 → ~1,130 and `app.py`'s `conn.execute` **42 → 12** — that second number is the one to judge P3 by, since 69 routes at ~10 lines each is a ~700-line floor extraction cannot go below. Decisions taken while planning that §4/§5 did not settle: the golden DB is a **plain copy** of `symr.db`, not the sampled one P2 imagined (an FK-consistent subset across ~20 tables is real engineering for something P3 deletes, and a snapshot of a page reading "no results" proves nothing); the passes run **under pytest** rather than through `golden.py`'s `__main__`, since `conftest.py` already supplies the frozen clock, blocked sockets and connect guard the CLI has none of; and **capture happens exactly once**, because re-capturing mid-way promotes any regression already introduced into the new baseline. §3.2 names the four things that must be dead before a byte diff means anything — chief among them the async scoring worker, which lands mid-pass and changes every page rendered after it. `canonical_index` is the one view with no clean home and the reasoning is in §4.1.1: it goes to `canonical_detect.py` (which already owns `canonical_page_groups` and `filter_groups` for that same page) with the two autogroup-status values left in the route, because `canonical_autogroup` imports `canonical_detect` and pulling them in would create a **new** cycle in the step whose other job is removing one. Two additions to the four findings: `_board_state`'s and the two `snapshot` sites' `SELECT *` (§4.5 — the riskiest change in P3, since `_board_state` `dict(row)`s straight into an API payload, and exactly what golden snapshots exist to catch), and a **narrow module-list check** for `CLAUDE.md` rather than the numeric-claim test that was considered and rejected in §4.3 for greping English that gets reworded. A seven-category dead-code sweep re-run 2026-08-22 found **nothing beyond `all_candidate_groups`** — zero unused imports, orphan templates, unused macros, unreferenced JS, unreferenced config settings or unused CSS classes, and the one sanctioned `TODO` — so P3 carries no cleanup backlog past what P1 already named. |
