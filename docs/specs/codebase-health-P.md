...
**The corollary, found the hard way in P2 session 1: a test can be neither of these and still be
green.** Both kinds above assume the assertion would *notice* a wrong answer. A test citing a real
clause, asserting something true, can still be one a broken implementation would satisfy — usually
because the fixture is too simple to separate two mechanisms that agree on the easy input. That is
invisible to the runner by construction, so it has to be caught at write time by asking what a
wrong implementation would have produced. It bites hardest on orderings, fallbacks, and
which-pass-handled-this questions. `docs/codebase-health/P2_tests.md` §1 carries the two real
examples and the fixture changes that fixed them; the same trap applies to any characterization
test P3 regenerates.
