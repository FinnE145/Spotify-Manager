...
- **Every spec carries a `## Tests` section**, naming what the suite should assert about this feature — the clauses worth pinning, not a test plan. It may legitimately say **"none — templates and JS only"** with the reason; a section that can honestly be empty is not box-ticking, whereas one that must always list something becomes ritual. This is deliberately redundant with the Verify gate rather than elegant: a single point of failure is what it avoids (`docs/specs/codebase-health-P.md` §7).
